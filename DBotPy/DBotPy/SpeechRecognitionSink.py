...
class SpeechRecognitionSink(Sink):
    ...
    def write(self, pcm_bytes, user):
        ...
        for i, user_audio in enumerate(self.get_all_audio()):
            ...
            with wave.open(f'Downloads/SpeechRecognition/{user}.wav', 'wb') as wavfile:
            ...
            result = self.model.transcribe(f'Downloads/SpeechRecognition/{user}.wav', language=self.whisper_language)
            recognized_text = result["text"]
            self.text_callback(self, user, recognized_text)

            if recorded_time >= BUFFER_CLEAN_TIME:
                # self.audio_data[user].seek(0)
                # self.audio_data[user].truncate(0)
                stream = io.BytesIO()
